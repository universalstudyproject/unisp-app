...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    const loadUser = () => {
      ...
      if (!storedUser) {
        // Usa replace per evitare loop con il tasto indietro
        router.replace("/");
      } else {
        try {
          ...
          localStorage.removeItem("unisp_user"); // Pulisci se il dato è corrotto
          router.replace("/");
        }
      }
    };
    ...
    loadUser();
  }, [router.pathname]); // Si attiva ogni volta che cambi pagina
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-slate-100 font-inter">
      ...
      <main className="max-w-md mx-auto px-4 pt-6 pb-32">{children}</main>
