...
export default function AdminView({
  ...
  updateMembreField,
  createLog,
  triggerAutoEmail, // Aggiunta prop per l'automazione email
}) {
    ...
    Papa.parse(file, {
      ...
      complete: async (results) => {
        try {
          ...
          if (data.success) {
            // 1. REGISTRAZIONE LOG IMPORTAZIONE
            await createLog(
              "IMPORT_CSV",
              `Importazione completata: ${data.imported} nuovi membri, ${data.skipped} duplicati saltati. Nome file: ${file.name}`,
            );

            // 2. AVVIO AUTOMAZIONE EMAIL (1 minuto di attesa gestito dalla Dashboard)
            if (data.imported > 0) {
              triggerAutoEmail(data.imported);
            }

            alert(
              `✅ Importazione completata!\n\n- Nuovi membri inseriti: ${data.imported}\n- Duplicati saltati: ${data.skipped}\n\nL'invio dei codici QR ai nuovi membri avverrà automaticamente tra 1 minuto.`,
            );
  ...
  return (
    <div className="space-y-6">
        ...
        <label
          className={`flex items-center justify-center gap-3 w-full py-4 rounded-2xl font-black uppercase tracking-widest transition-all shadow-lg cursor-pointer ${
            isImporting
              ...
              : "bg-green-600 text-white shadow-green-600/20 hover:bg-green-500 active:scale-95"
          }`}
        >
          {isImporting ? (
          ...
          <span>
            {isImporting ? "Importazione in corso..." : "Importa Membri CSV"}
          </span>
          <input
            type="file"
            accept=".csv"
      ...
      <div className="space-y-3 pb-24">
        {membres.length === 0 ? (
          <p className="text-center text-slate-500 py-10 font-bold uppercase text-xs tracking-widest">
                    ...
                    {isCompactMode && s === "ATTIVO" ? (
                      <button
                        ...
                        className={`px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                          hasAuth
                            ? "bg-red-500/20 text-red-500 border border-red-500/30 shadow-[0_0_15px_-5px_rgba(239,68,68,0.5)]"
